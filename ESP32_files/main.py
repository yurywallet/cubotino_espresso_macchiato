...
def initialize_robot(debug):
    ...
    print("robot initialization: {}".format(
        "OK" if robot_init_status else "FAILED"))
...
def robot_solver(solution, debug, stop_btn, btn_ref):
    ...
    print("servo_status:{}".format(robot_status))
    ...
    while True:                           # infinite loop
        ...
        while sys.stdin in select.select([sys.stdin], [], [], 0)[0]:  # while case there is data on the uart buffer
            ...
            if '[' in ch:                                             # case the character is an open square bracket (start of a message content)
                ...
                if 'current_settings' in strMsg:                      # case the message string includes the 'current_settings' word 
                ...
                elif 'jog' in strMsg:                                 # case the message string includes the 'jog' word
                    data_start=strMsg.find('(')                       # string position for the open round square character (start of data content)
                    data_end=strMsg.find(')')                         # string position for the close round square character (end of data content)
                    data=strMsg[data_start+1:data_end]                # string is sliced to only keep the data content, e.g. "b,84"
                    parts=data.split(',')                             # split into servo id and target duty value
                    if parts[0]=='b':                                 # case the jog request is for the bottom servo
                        import Cubotino_servos as servo               # module that manages the servos actions
                        servo.jog_bottom(int(parts[1]))                # bottom servo is moved directly to the requested PWM duty
                    ...
                    if not robot_init_status:
                        print("ERROR: servo initialization failed; movement blocked")
                    elif sol_string_ready:                            # case the cube solution string is ready
                         ...
                         try:
                             ...
                             print("ERROR: servo solver failed: {}".format(ex))
                    else:
                        print("ERROR: start received before a valid solution")
