...
def jog_bottom(duty):
    """ Moves the bottom servo directly to an absolute PWM duty value, for live tuning from the GUI Settings window.
        Bypasses the normal open/close/home state machine (manual override, tuning only)."""

    global b_servo_stopped, b_servo_home, b_servo_CW_pos, b_servo_CCW_pos

    duty = max(20, min(134, duty))   # hard safety clamp to the widest possible servo range (large pw mode)
    b_servo.duty(duty)               # bottom servo moves directly to the requested position
    b_servo_stopped = True           # boolean of bottom servo at location the lifter can be operated
    b_servo_home = False             # position is no longer tracked as a known named position
    b_servo_CW_pos = False           # position is no longer tracked as a known named position
    b_servo_CCW_pos = False          # position is no longer tracked as a known named position
...
def update_moves(start_moves, remaining_moves, index, debug=False):
    ...
    print('servo_step_done:{} remaining:{}'.format(index, remaining_moves))
...
def servo_solve_cube(moves, debug, stop_btn, btn_ref):
    ...
    for i in range(string_len):                              # iteration over the characters of the moves string
        ...
        if moves[i] in ('F', 'S', 'R'):
            print('servo_step_start:{}{}'.format(moves[i], moves[i + 1]))
